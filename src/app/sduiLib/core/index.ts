...
function createServerDrivenUI<Schema = DefaultSchema>(config: Config<Schema>): ServerDrivenUI {
  ...
  const createServerDrivenElement = async (loadParams: LoadParams, options?: Record<string, any>): Promise<any> => {
    ...
    if (uiTree) return config.renderComponentTree(config, uiTree, options)
